...
comentarioCtl.mandar = async (req, res) => {
  ...
  const { comentario, fecha_publicacion } = req.body;
  const nuevoEnvio = {
    comentario,
    fecha_publicacion,
  };
...
comentarioCtl.traer = async (req, res) => {
  ...
  const lista = await sql.query("select * from comentarios where id_comentario =?", [
    ids,
  ]);
...
comentarioCtl.actualizar = async (req, res) => {
  ...
  await orm.comentario.findOne({ where: { id_comentario: ids } }).then((actualizar) => {
...
comentarioCtl.eliminar = async (req, res) => {
  const ids = req.params.id;
  const id = req.user.id_cliente;
  await orm.comentario.destroy({ where: { id_comentario: ids } }).then(() => {
